...
# Date:      2011.01.06 - 2010.08.05
# Python:    3.2
# Licence:   GPLv3, Copyright (c) 2010-2011 Ioannis Filippidis

# Based on:  InkscapeSlide 1.0
# by:        Alexandre Bourget
# License:   GPLv3 Copyright (c) 2008 by Alexandre Bourget
# URL:       http://pypi.python.org/pypi/InkscapeSlide/1.0
#            http://projects.abourget.net/inkscapeslide/
#            https://github.com/abourget/inkscapeslide
# Note:      Changes by Ioannis Filippidis:
...
def main():
    ...
    parser.add_option("-f", "--file", dest="svgfile",
        default=False, help="Provide input SVG file name")
    parser.add_option("-s", "--scene-order", dest="sceneorder",
        default=False, help="Define order of generated scenes for \onslide<> in .pdf_tex\
            as a comma-separated list like --scene-order=1-2,3,4,5-5,6-8")
    parser.add_option("-w", "--width", dest="width",
        default=False, help="Define width of included PDF scenes")
    parser.add_option("-i", "--imageexport", action="store_true", dest="imageexport",
        default=False, help="Use PNG files as export content")
    parser.add_option("-b", "--overprint", dest="overprint",
    ...
    if svg_exists == True:
        ...
        (mode, ino, dev, nlink, uid, gid, size, atime, svgmtime, ctime) = os.stat(FILENAME)
    else:
    ...
    content_layer = [x for x in layers
        if x.attrib.get('{http://www.inkscape.org/namespaces/inkscape}label', False).lower() == 'content']
    ...
    preslides = [x.text for x in
        content.findall('{http://www.w3.org/2000/svg}text/{http://www.w3.org/2000/svg}tspan') if x.text]
    ...
    for i, slide_layers in enumerate(slides):
        ...
        svgslide = os.path.abspath(os.path.join(os.curdir,"%s_scene%d.svg" % (fname, i)))
        pdfslide = os.path.abspath(os.path.join(os.curdir,"%s_scene%d.pdf" % (fname, i)))
        pdftexslide = "%s_scene%d.pdf_tex" % (fname, i)
        
        # Use the correct extension if using images
        if options.imageexport:
            pdfslide = os.path.abspath(os.path.join(os.curdir,".inkscapeslide_%s.p%05d.png" % (fname, i)))
        ...
        if pdf_exists == True:
            ...
            # get last modification time
            (mode, ino, dev, nlink, uid, gid, size, atime, pdfmtime, ctime) = os.stat(pdfslide)
            print("\n------------------------\nlast mod dates?\n\tSVG: %s\n\tPDF: %s\n"
        ...
        if (flag == 0) or (svgmtime > pdfmtime):
            ...
            f = open(svgslide, 'wb')
            f.write(lxml.etree.tostring(doc, encoding='UTF-8', xml_declaration=True))
            f.close()
            
            # Determine whether to export pdf's or images (e.g. inkscape -A versus inkscape -e)
            cmd = "inkscape -z -D --export-latex -A %s %s" % (pdfslide, svgslide)
            if options.imageexport:
            ...
            #p = subprocess.call(args); # wait for export to prevent latex going on
            #while inkscape is still working
    ...
    if len(scene_order) != len(pdftexslides):
        print("Number of scenes generated is different than defined in scene order.")
        print("No .pdf_tex file created.")
        sys.exit(1)
